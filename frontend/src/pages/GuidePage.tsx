...
import { clampRecommendedAi } from "../lib/types";
...
export default function GuidePage() {
  ...
  }, [tasks, guides.length, projectInput, setGuides, approvedAi]);
  ...
  return (
    <section className="max-w-3xl">
      ...
      <p className="mt-3 max-w-2xl text-sm text-ink-700/75">
        승인된 AI 안에서 업무별로 무엇을 어떻게 쓸지 안내합니다. (헤더의
        「승인 AI 관리」에서 변경)
      </p>
      ...
      <div className="mt-8 grid gap-4">
        {guides.map((g) => (
          <article key={g.task_id} className="border border-ink-200 bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <h2 className="text-base font-semibold text-ink-900">{g.task_name}</h2>
              <VerdictBadge verdict={g.verdict} />
            </div>
            <p className="mt-3 text-sm font-medium text-accent-dark">
              {g.recommended_ai} · {g.guide}
            </p>
            <p className="mt-2 text-sm leading-relaxed text-ink-700/75">{g.how}</p>
          </article>
        ))}
      </div>
